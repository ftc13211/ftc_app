...
public class MecanumTeleop extends OpMode {
    ...
    private boolean previousX2 = false; //for emergency stop of relic arm
    private boolean relicEnabled = false;
    ...
    public void loop() {
		...
		*/
        ...
        if (gamepad2.x && !previousX2) {
            relicEnabled = !relicEnabled;
        }
        if (relicEnabled && gamepad2.right_trigger > 0.1 && robot.relicMotor.getCurrentPosition() < RELIC_MAX_POS) {
            robot.relicMotor.setPower(1.0);
        } else if (relicEnabled && gamepad2.left_trigger > 0.1 && robot.relicMotor.getCurrentPosition() > RELIC_MIN_POS) {
            robot.relicMotor.setPower(-1.0);
        } else if (relicEnabled && gamepad2.right_bumper && robot.relicMotor.getCurrentPosition() > RELIC_MIN_POS) {
            robot.relicMotor.setPower(0.4);
        } else if (relicEnabled && gamepad2.left_bumper && robot.relicMotor.getCurrentPosition() > RELIC_MIN_POS) {
            robot.relicMotor.setPower(-0.4);
        } else {
            robot.relicMotor.setPower(0.0);
        }
        previousX2 = gamepad2.x;
        telemetry.addData("RELIC ENABLED", relicEnabled);
